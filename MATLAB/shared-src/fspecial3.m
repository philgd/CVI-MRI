...
%   filter of size SIZE. If SIG is not specified, it defaults to
...
%   Gaussian filter of size SIZE. If SIG is not specified, it defaults to
%   SIZE/(4*sqrt(2*log(2))).
%
%   Class Support
%   -------------
...
if numel(sig)==1
    sig = repmat(sig,1,3);
elseif numel(sig)~=3
    error('Number of elements in SIG must be 1 or 3')
end
